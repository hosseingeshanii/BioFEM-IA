...
PetscErrorCode FungJacobian(PetscInt ibi, FE* fem, PetscReal epsilon){
  ...
  */
  ...
  for (int m = 0; m < n_Fung_Coeffs-2; m++){
    for(ec=0; ec<ibm->n_elmt; ec++){        
  ...
  if (uniform_fiber_dir){
    for (int m = n_Fung_Coeffs-2; m < n_Fung_Coeffs - 1; m++){
      for(ec=0; ec<ibm->n_elmt; ec++){              
...
PetscErrorCode FungUniJacobian(PetscInt ibi, FE* fem, PetscReal epsilon){
  ...
  PetscPrintf(PETSC_COMM_SELF, "Uniform Fung is selected\n");
  IBMNodes   *ibm=fem->ibm;
  PetscInt   nv, ec;
  PetscScalar *RRes;
  ...
  // nonuniform fiber
  if (!uniform_fiber_dir){
    PetscPrintf(PETSC_COMM_SELF, "Non uniform fiber direction is selected\n");
    for (int m = 7; m < 8; m++){
      for(ec=0; ec<ibm->n_elmt; ec++){       
